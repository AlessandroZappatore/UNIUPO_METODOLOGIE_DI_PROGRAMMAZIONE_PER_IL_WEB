...
//Setup multer per salvare i poster nella cartella poster
...
router.post('/aggiungi_contenuto', posterUpload.single('poster'), [
    ...
    check('Data_uscita').optional({ checkFalsy: true }).isDate().withMessage('Inserisci una data di uscita valida'),
    ...
    if (!poster) {
        console.error('Errore durante il caricamento del poster');
        return res.status(400).render("error", { message: "Errore durante il caricamento del poster", buttonAction: "Back" });
    }
    ...
    try {
        ...
        console.error('Errore durante la creazione del contenuto:', error);
    ...
    try {
        ...
        fs.unlink(posterPath, (err) => {
            if (err) {
                ...
                return res.render("error", { message: "Errore durante l'eliminazione del poster", buttonAction: "Back" });
            }
            else
